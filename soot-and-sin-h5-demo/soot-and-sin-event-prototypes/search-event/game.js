...
async function startSearchFilm(selectedId, chances) {
  ...
  try {
    await Promise.all(searchAnimations.map((animation) => animation.finished));
  } catch {
    return false;
  }
  elements.focusTrack
...
function resetEvent({ openVote = false } = {}) {
  ...
  updateChanceDisplay();
  if (openVote) schedule(playEntrance, 250);
}
...
function handleSuccess(rewardTier) {
  const rewards = mergeLoot([], generateRewards(rewardTier));
  const rewardName = searchSlides.find((slide) => slide.id === rewardTier)?.name || "物資";
  mergeLoot(state.loot, rewards);
...
function startResultCountdown() {
  ...
  schedule(() => {
    if (state.phase === "result") prepareNextVote();
  }, 3000);
}

function mergeLoot(target, incoming) {
  incoming.forEach((item) => {
    const stack = target.find((entry) => entry.id === item.id);
    if (stack) {
      stack.quantity += item.quantity;
      stack.depth = Math.min(stack.depth, item.depth);
    } else {
      target.push({ ...item });
    }
  });
...
function openDistribution() {
  ...
  state.playerSelections.clear();
  state.allySelections.clear();
  state.results = [];
  state.allyReady = false;
  elements.lootModal.classList.add("open");
  elements.lockButton.hidden = false;
  elements.lockButton.disabled = true;
  elements.lockButton.textContent = "等待 Morrow 選擇";
  elements.continueButton.hidden = true;
  elements.lootSummary.className = "loot-summary";
  elements.lootSummary.textContent = "";
...
function renderLoot() {
  elements.lootGrid.innerHTML = state.loot.map((item) => {
    const playerSelected = state.playerSelections.has(item.instanceId);
    const allySelected = state.allySelections.has(item.instanceId);
    const result = state.results.find((entry) => entry.instanceId === item.instanceId);
    const resultClass = result
      ? result.winner
        ? `resolved won-${result.winner}`
        : "resolved discarded"
      : "";
    const winnerText = result
      ...
      : "";
    return `
      <button class="loot-item rarity-${item.rarity} ${playerSelected ? "selected" : ""} ${resultClass}"
        data-loot="${item.instanceId}" type="button" ${state.phase === "resolved" ? "disabled" : ""}>
        <span class="loot-art"><svg aria-hidden="true"><use href="#icon-${item.icon}"></use></svg></span>
        <span class="loot-copy">
          <small>${rarityNames[item.rarity]} · 深入 ${item.depth} 發現</small>
          <h3>${item.name}</h3>
          <p>${item.stat}</p>
        </span>
        <b class="loot-quantity">×${item.quantity}</b>
        <span class="claimants">
          ${playerSelected ? claimantMarkup(actors.player) : ""}
          ${allySelected ? claimantMarkup(actors.ally) : ""}
        </span>
        ${winnerText ? `<span class="winner-tag">${winnerText}</span>` : ""}
      </button>
    `;
  }).join("");
}

function claimantMarkup(actor) {
  return `
    <span class="claimant ${actor.id === "ally" ? "ally" : ""}" title="${actor.name}">
...
function chooseAllyLoot() {
  const shuffled = [...state.loot].sort(() => Math.random() - .5);
  const count = Math.min(state.loot.length, Math.floor(Math.random() * 4));
  const picks = shuffled.slice(0, count);
  if (!picks.length) {
    schedule(() => {
      if (state.phase !== "distribution") return;
      state.allyReady = true;
      elements.lockButton.disabled = false;
      elements.lockButton.textContent = "確認選擇";
      elements.lootSummary.className = "loot-summary visible";
      elements.lootSummary.textContent = "Morrow 決定不爭取任何物資。";
    }, 700);
    return;
  }
  picks.forEach((item, index) => {
    schedule(() => {
      if (state.phase !== "distribution") return;
      state.allySelections.add(item.instanceId);
      renderLoot();
      if (index === picks.length - 1) {
        ...
        elements.lockButton.textContent = "確認選擇";
        elements.lootSummary.className = "loot-summary visible";
        elements.lootSummary.textContent = `Morrow 已選擇 ${picks.length} 組物資。`;
      }
    }, 550 + index * 420);
  });
}
...
function toggleLoot(instanceId) {
  if (state.phase !== "distribution") return;
  if (state.playerSelections.has(instanceId)) {
    state.playerSelections.delete(instanceId);
  } else {
    state.playerSelections.add(instanceId);
  }
  renderLoot();
}
...
function resolveDistribution() {
  ...
  state.results = state.loot.map((item) => {
    const selectors = [];
    if (state.playerSelections.has(item.instanceId)) selectors.push("player");
    if (state.allySelections.has(item.instanceId)) selectors.push("ally");
    return {
      ...item,
      winner: selectors.length
  ...
  const discarded = state.results.filter((item) => !item.winner);
  elements.lootSummary.className = "loot-summary visible";
...
function continueEvent() {
  const detail = {
    ...
    depth: state.depth,
    successes: state.successes,
    playerLoot: state.results.filter((item) => item.winner === "player"),
...
elements.lootGrid.addEventListener("click", (event) => {
  const item = event.target.closest("[data-loot]");
  if (item) toggleLoot(item.dataset.loot);
});
...
elements.replayButton.addEventListener("click", () => {
  eventIndex += 1;
  resetEvent({ openVote: true });
});
elements.battleReplayButton.addEventListener("click", () => {
  eventIndex += 1;
  resetEvent({ openVote: true });
});
