...
const DEFAULT_DEFINITION_PATH := "res://config/bundles/bootstrap_files.json"
const ALLOWED_ROOT := "res://content/bundles/"
...
func _init() -> void:
	...
	if not bool(definition_result.get("ok", false)):
		_fail(str(definition_result.get("error", "Invalid bundle definition.")))
		return
	...
	if directory_error != OK:
		_fail("Cannot create bundle directory: %s" % error_string(directory_error))
		return
	if FileAccess.file_exists(output_path):
		var remove_error := DirAccess.remove_absolute(output_path)
		if remove_error != OK:
			_fail("Cannot replace previous bundle: %s" % error_string(remove_error))
			return

	var packer := PCKPacker.new()
	var start_error := packer.pck_start(output_path)
	if start_error != OK:
		_fail("Cannot create PCK: %s" % error_string(start_error))
		return
	...
	if flush_error != OK:
		_fail("Cannot finish PCK: %s" % error_string(flush_error))
		return
	print("Wrote data-only content pack with %d file(s): %s" % [files.size(), output_path])
	quit()
...
func _read_definition(path: String) -> Dictionary:
	...
	if json.parse(text) != OK or typeof(json.data) != TYPE_DICTIONARY:
		return {"ok": false, "error": "Cannot parse bundle definition: %s" % path}
	var definition: Dictionary = json.data
	if int(definition.get("schema_version", -1)) != 1:
		return {"ok": false, "error": "Unsupported bundle definition schema."}

	var files_variant: Variant = definition.get("files", [])
	if typeof(files_variant) != TYPE_ARRAY or files_variant.is_empty():
		return {"ok": false, "error": "Bundle definition must list at least one file."}
	...
	for virtual_path_variant in files:
		...
		if (
			...
			return {"ok": false, "error": "Unsafe bundle path: %s" % virtual_path}
		if virtual_path.get_extension().to_lower() in FORBIDDEN_EXTENSIONS:
			return {"ok": false, "error": "Executable content is forbidden: %s" % virtual_path}
		if seen.has(virtual_path):
			return {"ok": false, "error": "Duplicate bundle path: %s" % virtual_path}
		if not FileAccess.file_exists(virtual_path):
			return {"ok": false, "error": "Bundle source is missing: %s" % virtual_path}
